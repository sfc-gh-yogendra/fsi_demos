...
- ✅ **SQL-Based**: All agents created using `CREATE OR REPLACE AGENT` statements
- ✅ **Automated**: Executed automatically via `python main.py`
- ✅ **Centralized**: All agent creation logic in `python/create_agents.py`
- ✅ **Properly Formatted**: Instructions use YAML escaping (`\n` for newlines, `\"` for quotes)
...
def verify_ai_components_for_agent(session: Session, required_services: List[str]):
    ...
    try:
        ...
        print("✅ Semantic view: SAM_ANALYST_VIEW")
...
"""
    ...
    print("✅ Created agent: portfolio_copilot")
```
...
### 2.4 Adding New Agents - Complete Workflow

**To add a new agent, follow this comprehensive workflow that uses all patterns from this guide:**

#### Step 1: Design Agent Instructions (Using Patterns from Step 3)

**Response Instructions** - Use pattern from Section 3.2.1:
```python
def get_new_agent_response_instructions():
    """Get response instructions for new agent"""
    return """Style:
- Tone: [Specific tone for persona - see Section 3.2.1]
- Lead With: [Answer pattern - see Section 3.2.1]
- Terminology: [Domain-specific terms - see Section 3.2.1]
- Precision: [Detail level - see Section 3.2.1]
- Limitations: [How to communicate gaps - see Section 3.2.1]

Presentation:
- Tables: Use for [criteria with threshold - see Section 3.2.1]
- Bar Charts: Use for [criteria - see Section 3.2.1]
- Line Charts: Use for [criteria - see Section 3.2.1]
- Single Metrics: Format as "[pattern]"
- Data Freshness: Always include "[pattern]"

Response Structure for [Common Query Type 1]:
Template: "[Structure with placeholders - see Section 3.2.1 for examples]"

Example:
User: "[realistic question]"
Response: "[complete example showing all elements]"
...
**Orchestration Instructions** - Use patterns from Sections 3.1.1, 3.1.2, 3.1.3:
```python
...
- [Organization details - see Section 3.1.1]
- [Key metrics - see Section 3.1.1]
- [Regulatory framework - see Section 3.1.1]
- [Data operations - see Section 3.1.1]

Key Business Terms:
- [Term 1]: [Definition with thresholds - see Section 3.1.1]
- [Term 2]: [Definition with thresholds - see Section 3.1.1]
- [Term 3]: [Definition with process - see Section 3.1.1]

[Domain] Categories:
- [Category 1]: [Characteristics - see Section 3.1.1]
- [Category 2]: [Characteristics - see Section 3.1.1]

Tool Selection Strategy:

1. [Query categorization - see Section 3.1]
2. [Tool selection logic with ✅/❌ examples]
3. [Workflow sequencing for multi-tool queries]

Complete Workflow Examples:

Workflow 1: [Workflow Name]
Trigger: User asks "[example question]"

Step-by-Step Execution:
1. [First Action]
   Tool: [tool_name]
   Query/Search: "[exact pattern]"
   Extract from results: [specific info]
   Why this step: [business justification]

2. [Second Action]
   Tool: [tool_name]
   Query/Search: "[exact pattern using Step 1 results]"
   Extract from results: [specific info]

3. [Third Action]
   [Continue pattern - see Section 3.1.2 for complete example]

4. Synthesize Final Response:
   - [Element 1]: From Step [X]
   - [Element 2]: From Step [Y]
   - Format as: [specific format]
   - Include: [required elements]

Example Complete Interaction:
User Question: "[realistic question]"
Agent Response: "[complete response showing all workflow elements]"

Workflow 2: [Another Workflow]
[Follow same pattern - see Section 3.1.2]

Error Handling and Edge Cases:

Scenario 1: [Error Condition]
Detection: [How to identify - see Section 3.1.3]
Recovery Steps:
  1. [Recovery action 1]
  2. [Recovery action 2]
  3. [Final fallback]
User Message: "[Exact message pattern]"
Alternative: [What to suggest]

Scenario 2: [Different Error]
[Follow same pattern - see Section 3.1.3 for 5 complete examples]

[Continue for 3-5 scenarios]
"""
```

#### Step 2: Configure Tools (Using Patterns from Section 2.2)

**For Cortex Analyst tools** - Use pattern from Section 2.2.1:
- Include Data Coverage (5 elements)
- Include Semantic Model Contents (4 elements)
- Include When to Use (3-4 examples)
- Include When NOT to Use (3 anti-patterns with alternatives)
- Include Query Best Practices (3+ with ✅/❌ examples)

**For Cortex Search tools** - Use pattern from Section 2.2.1:
- Include Data Sources (5 elements)
- Include When to Use (3-4 examples)
- Include When NOT to Use (3 anti-patterns with alternatives)
- Include Search Query Best Practices (3+ with ✅/❌ examples)

#### Step 3: Implement Agent Creation Function

```python
def create_new_agent(session: Session):
    """Create new agent with comprehensive tool descriptions."""
    database_name = config.DATABASE['name']
...
CREATE OR REPLACE AGENT SNOWFLAKE_INTELLIGENCE.AGENTS.new_agent
  ...
  tools:
    - tool_spec:
        type: "cortex_analyst_text_to_sql"
        name: "[tool_name]"
        description: "[Use comprehensive pattern from Section 2.2.1 - include Data Coverage, When to Use/NOT, Query Best Practices]"
    - tool_spec:
        type: "cortex_search"
        name: "search_[document_type]"
        description: "[Use comprehensive pattern from Section 2.2.1 - include Data Sources, When to Use/NOT, Search Best Practices]"
  tool_resources:
    [tool_name]:
      execution_environment:
        query_timeout: 30
    ...
    search_[document_type]:
      search_service: "{database_name}.AI.SAM_[SERVICE_NAME]"
...
"""
    ...
    session.sql(sql).collect()
    print("✅ Created agent: new_agent")
```

#### Step 4: Register Agent

**Add to `get_agent_instructions()` function**:
```python
...
- [ ] Use **Section 2.2.1** for comprehensive tool descriptions
- [ ] Use **Section 3.1.1** for business context patterns
- [ ] Use **Section 3.1.2** for explicit workflow examples
- [ ] Use **Section 3.1.3** for error handling patterns
- [ ] Use **Section 3.2.1** for structured response instructions
- [ ] Reference **@agent_configuration_checklist.md** for systematic validation
- [ ] Reference **@agent_config_enhanced_example.md** for complete reference template
...
4. FLAGGING AND THRESHOLDS: Include specific flagging requirements based on agent type:
   - Portfolio agents: Flag positions >6.5% with "⚠️ CONCENTRATION WARNING"
   - Compliance agents: Flag breaches >7% with "🚨 BREACH" and warnings >6.5% with "⚠️ WARNING"
...
- Single Metrics: Format as "{pattern}"
  Example: "Portfolio weight is 8.2% (⚠️ exceeds 6.5% threshold) as of 31 Dec 2024"
- Data Freshness: Always include "{pattern}"
...
def test_agent_components(session: Session, agent_name: str, tools: List[str]):
    ...
    for analyst_tool in analyst_tools:
        try:
            ...
            """).collect()
            print(f"✅ Cortex Analyst tool test passed: {analyst_tool} ({len(result)} results)")
        except Exception as e:
            print(f"❌ Cortex Analyst tool test failed for {analyst_tool}: {e}")
    ...
    for search_tool in search_tools:
        ...
        try:
            ...
            """).collect()
            print(f"✅ Cortex Search tool test passed: {search_tool}")
        except Exception as e:
            print(f"❌ Cortex Search tool test failed for {search_tool}: {e}")
    
    # Validate agent has at least one working tool
    if not analyst_tools and not search_tools:
        print(f"❌ Agent {agent_name} has no valid tools configured")
        return False
...
- Single Metrics: Format as 'Weight is 8.2% (⚠️ exceeds 6.5%) as of 31 Dec 2024'
...
❌ **No Response Structure Templates**
```yaml
...
[table]
⚠️ 3 positions exceed 6.5% threshold
Total flagged: 22.4%'"
```
...
- [ ] For Cortex Analyst: Add Query Best Practices with ✅/❌ examples
