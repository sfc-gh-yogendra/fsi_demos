...
def create_search_services(session: Session, scenarios: List[str]):
    ...
    # This corpus is created by unstructured data generation, so only create if it exists
    if 'portfolio_copilot' in scenarios or 'mandate_compliance' in scenarios:
        try:
            ...
            # Check if corpus table exists (created by unstructured data generation)
            ...
            print(f"WARNING:  Report templates corpus not yet created (will be generated by unstructured data pipeline)")
            # print(f"   Search service SAM_REPORT_TEMPLATES will be created in next build phase")
